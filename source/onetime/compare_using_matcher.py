...
def main(args):
    ...
    print('    ', end='')
    for i in range(nrof_images):
        _, top_match_ids, dists = matcher.match(
            emb[i, :], top_matches=57, return_min_dist=True)
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        '--image_dir', type=str, help='Directory containing images to compare')
    parser.add_argument(
        '--image_files',
        type=str,
        default=None,
    ...
    parser.add_argument(
        '-d', '--display', help='choose to display image', action='store_true')
