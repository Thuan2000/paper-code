...
from cv_utils import (calc_bb_size, PickleUtils, clear_tracking_folder,
                      is_inner_bb)
...
def draw_results(image, detect_results):
    ...
    for result in detect_results:
        ...
        cv2.rectangle(tmpi, (x1, y1), (x2, y2), (0, 255, 0), 3)

        cv2.putText(tmpi, str_out, (int(x1 + (x2 - x1) / 2), y1),
        ...
        while True:  # frame_reader.has_next():
            ...
            if frame_counter % Config.Frame.FRAME_INTERVAL == 0:
                ...
                for result in results:
                    ...
                    body_image = frame[origin_bb[1]:origin_bb[3], origin_bb[0]:
                                       origin_bb[2], :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--image', help='your image', default=None)
    parser.add_argument(
        '-a',
        '--area',
