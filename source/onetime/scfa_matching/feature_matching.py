...
def keypoint_matching(query_image, train_image):
    ...
    if len(good_matches) > MIN_MATCH_COUNT:
        ...
        points = np.float32([[0, 0], [0, h - 1], [w - 1, h - 1],
                             [w - 1, 0]]).reshape(-1, 1, 2)
        dst = cv2.perspectiveTransform(points, M)
        train_image = cv2.polylines(train_image, [np.int32(dst)], True, 255, 3,
                                    cv2.LINE_AA)
    else:
        matches_mask = None
    ...
    draw_params = dict(
        ...
        flags=2)
    output = cv2.drawMatches(query_image, kp1, train_image, kp2, good_matches,
                             None, **draw_params)
