...
def main():
    ...
    while True:
        ...
        if len(faces) == 0 or len(faces) == 2:
            reason = "many faces in camera" if len(
                faces) else "your face is unclear"
            cv2.putText(image, reason, (2, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
        ...
        cv2.rectangle(image, (face.left(), face.top()),
                      (face.right(), face.bottom()), (255, 0, 255), 1,
                      cv2.LINE_AA)

        # verification direction
        if verification_mode:
            direction = compute_direction(image, face, rules[pointer], [
                predictor, model_points, camera_matrix, dist_coeffs, directions
            ])
            cv2.putText(image, "put your head " + rules[pointer], (2, 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2,
                        cv2.LINE_AA)
            cv2.putText(image, "x-y directions %s" % direction, (2, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2,
                        cv2.LINE_AA)

        # display image
        if verification_result == 1:
            cv2.putText(image, "verification succeeds", (200, 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2,
                        cv2.LINE_AA)
        elif verification_result == -1:
            cv2.putText(image, "verification fails", (200, 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2,
                        cv2.LINE_AA)
        cv2.imshow("output", image)
