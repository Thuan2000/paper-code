...
from cv_utils import encode_image, decode_image, encode_ndarray, decode_ndarray
...
class RabbitMQ:
    ...
    # send queue to thecoffeehouse.eyeq.tch
    # SEND_QUEUE_TCH_LIVE_RESULT = 'vingroup-ci'

    # 4. Import profiles in batch
    SEND_QUEUE_IMPORT_REGISTER = prefix + 'eyeq-vingroup-cam-register-result'

    # loingo add 08/05/2018
    TCH_TOP_MATCH_API = prefix + 'hieuha-test-tch'
    """
    """

    def __init__(self,
                 logins=(Config.Rabbit.USERNAME, Config.Rabbit.PASSWORD),
    ...
    @catch_connection_closed
    def send_with_exchange(self, exchange, routing_key, msg):
        """
        >>> rb = RabbitMQ()
        >>> rb.send('loi-rabbitmq-test-queue', 'test-string')
        message sent
        ...
        if method_frame:
            ...
            try:
                ...
                with open('../data/er_log.txt', 'a') as f:
                    f.write('Fail message: {}\n'.format(body))
                return None
            except:
                print('Something wrong here :v')
                with open('../data/er_log.txt', 'a') as f:
                    f.write('Fail message: {}\n'.format(body))
        else:
            return None
    ...
    @catch_connection_closed
    def receive_str(self, queue_name):
        method_frame, header_frame, body = self.channel.basic_get(
            queue=queue_name, no_ack=True)
        if method_frame:
    ...
    def send_recognition_result_tch(self, human_id, image, time_stamp,
                                    queue_name):
        """
        >>> from cv_utils import sample_pixel
        >>> rb = RabbitMQ()
    ...
    def is_exist(self, queue_name):
        '''
        Check if queue_name exist
        :param queue_name: name of queue to check
        :return bool: True if queue exist, false otherwise
        >>> rb = RabbitMQ()
        >>> rb.is_exist('non-exist')
        False
        >>> rb.is_exist(rb.RECEIVE_CAM_WORKER_QUEUE)
        True
        '''
        try:
    ...
    def receive_confirm_message(self, queue_name):
        # format token|confirmed_id
        return_dict = {}
        msgs = self.receive(queue_name)
        for msg in msgs:
            return_dict[msg[0]] = msg[1]
        return return_dict

    # loingo added 08/05/2018 for top 3 tch-api
    def send_top_matching_results(self, top1_image_id, top1_image_url,
                                  top_matching_list):
        msg_list = [top1_image_id, top1_image_url]
        for face_id, image_url in top_matching_list:
            msg_list.append(Config.INTRA_SEP.join([face_id, image_url]))
        msg = Config.INTER_SEP.join(msg_list)
        self.send(self.TCH_TOP_MATCH_API, msg)
