...
def populate_table(table_name, records):
    conn = r.connect(
        '210.211.119.152', 28015, user='tch', password='tch', timeout=3600)
...
def insert_to_clone(table_name, records):
    conn = r.connect(
        '210.211.119.152', 28015, user='admin', password='admin', timeout=3600)
    prev_id = -2
    while True:
        time.sleep(2)
        if len(records) > prev_id + 1 and len(records) > 0:
            r.db('TCH').table(table_name).insert(
                records[len(records) - 1]).run(conn)
        ...
        if hit_times < 5:
            ...
            if change['visitor_type'] == 'old':
                try:
                    ...
                    continue
                capture_img_url = "http://210.211.119.152:5000/" + change[
                    'captured']
                print(capture_img_url)
                save_image(capture_img_url, 'captured.jpg',
                           os.path.join(image_dir, token))
                for match in change['matched']:
                    if 'NOT_FOUND' not in match['image']:
                        match_id = match['face_id']
                        match_img_url = "http://210.211.119.152:5000/" + match[
                            'image']
                        print(match_img_url)
                        save_image(match_img_url, match_id + '.jpg',
                                   os.path.join(image_dir, token))
