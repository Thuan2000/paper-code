...
import argparse
import numpy as np
from cv_utils import get_new_key, get_key_by_value
...
def main(args):
    ...
    groundtruths_raw = pickle.load(
        open(os.path.join(database_dir, 'groundtruth.pkl'), 'rb'))
    predictions_raw = pickle.load(open(predictions_file, 'rb'))
    frames_dict = pickle.load(
        open(os.path.join(database_dir, 'frames.pkl'), 'rb'))
    ...
    groundtruth_non_faces = set(
        key for key, value in groundtruths.items() if 'non_face' in value)
    ...
    new_true_predict_as_non_face = groundtruth_non_faces.difference(
        predictions_keys)
    ...
    print('recognition precision:',
          1 - len(false_recognitions) / len(available_image_ids))
    ...
    if DEGUG_MODE:
        ...
        for image_id in false_recognitions:
            ...
            groundtruth_image_id = get_key_by_value(groundtruth_id,
                                                    reg_image_face_dict)
            predict_image_id = get_key_by_value(predict_id, reg_image_face_dict)
            if groundtruth_image_id and predict_image_id:
                origin_pkl_file = os.path.join(live_dir,
                                               origin_image_id + '.pkl')
                input_emb = pickle.load(open(origin_pkl_file, 'rb'))[1]

                groundtruth_pkl_file = os.path.join(
                    live_dir, groundtruth_image_id + '.pkl')
                groundtruth_emb = pickle.load(open(groundtruth_pkl_file,
                                                   'rb'))[1]

                predict_pkl_file = os.path.join(live_dir,
                                                predict_image_id + '.pkl')
                predict_emb = pickle.load(open(predict_pkl_file, 'rb'))[1]

                gt_dist = np.sum(
                    np.square(np.subtract(input_emb, groundtruth_emb)))
                pt_dist = np.sum(np.square(np.subtract(input_emb, predict_emb)))
                print(image_id, groundtruth_id, predict_id, gt_dist, pt_dist)
                result_list.append([
                    origin_image_id, groundtruth_id, predict_id, gt_dist,
                    pt_dist
                ])
            # print(groundtruth_image_id, predict_image_id)
            live_tup = pickle.load(
                open(os.path.join(live_dir, origin_image_id + '.pkl'), 'rb'))
    ...
    if WRITE_MODE:
        ...
        for image_id in false_recognitions:
            ...
            if groundtruth_id in One_Person_Multi_IDs:
                One_Person_Multi_IDs[groundtruth_id].append((image_id,
                                                             predict_id))
            else:
                One_Person_Multi_IDs[groundtruth_id] = [(image_id, predict_id)]

            if predict_id in Multi_Person_One_IDs:
                Multi_Person_One_IDs[predict_id].append((image_id,
                                                         groundtruth_id))
            else:
    ...
    # writing to file
        ...
        One_Person_Multi_IDs_dir = os.path.join(save_dir,
                                                'One_Person_Multi_IDs')
        Multi_Person_One_IDs_dir = os.path.join(save_dir,
                                                'Multi_Person_One_IDs')
        ...
        for groundtruth_id, image_id_list in One_Person_Multi_IDs.items():
            ...
            for image_id, predict_id in image_id_list:
                ...
                (img, _) = pickle.load(
                    open(os.path.join(live_dir, image_id + '.pkl'), 'rb'))
...
def parse_agruments(argv):
    parser = argparse.ArgumentParser(
        'Parses agrument for compare script',
    ...
    parser.add_argument(
        '-w', '--write', help='write files or not', action='store_true')
    parser.add_argument(
        '-d', '--debug', help='write files or not', action='store_true')
