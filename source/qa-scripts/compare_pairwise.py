...
import argparse
import numpy as np
from cv_utils import chop_extension, chop_n_element_from_end, calc_iou
...
def find_original_image_id(input_image_id,
                           gt_frame_image_dict,
                           use_origin_bb=False):
...
def main(args):
    ...
    for label_dir in gt_label_dirs:
        ...
        for image_path in image_paths:
            image_id = chop_n_element_from_end(chop_extension(image_path), 9)
    ...
    d = os.path.join(test_data_dir, '**', '*')
    td_image_paths = glob.glob(d)
    test_data_dict = {}
    td_bad_tracks = []
    missing_images = 0
    ...
    for image_path in td_image_paths:
        if 'match' not in image_path.lower():
            ...
            _image_id = chop_n_element_from_end(chop_extension(image_path), 9)
            image_id = find_original_image_id(_image_id, gt_frame_image_dict,
                                              args.use_origin_bb)
            if args.verbose:
    ...
    interest_images = list(
        set(groundtruth_dict.keys()).intersection(test_data_keys))
    false_positive_in_test_data = list(
        set(gt_false_positive).intersection(test_data_keys))
    unknown_images = test_data_keys.difference(
        set(interest_images).union(set(false_positive_in_test_data)))
    ...
    print('nrof not-in-groundtruth images',
          len(unknown_images) + missing_images)
    print('nrof pairs: ', nrof_sample)
    print('true positive: ', tp)
    print('false positive: ', fp)
    print('true negative: ', tn)
    print('false negative: ', fn)
    print('FAR: {}%'.format(round(fp / (fp + tn + 1e-6), 4) * 100))
    print('FRR: {}%'.format(round(fn / (fn + tp + 1e-6), 4) * 100))
    print('Precision: {}%'.format(round(tp / (tp + fp + 1e-6), 4) * 100))
    print('Recal base on test data: {}%'.format(
        round(tp / (tp + fn + 1e-6), 4) * 100))
    # recall is calculate by the nrof true prediction over nrof available pos pairs in gt
    print('Recall base on groundtruth: {}%'.format(
        round(tp / (nrof_gt_pos_pairs + 1e-6), 4) * 100))


def parse_arguments(argv):
    parser = argparse.ArgumentParser(
        'parser for pairwise compare',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--groundtruth', help='Ground truth folder')
    parser.add_argument(
        '--test_dir', help='new data directory after run algorithm')
    parser.add_argument(
        '--use_origin_bb',
        help='use original bb to generate unique key',
        action='store_true')
    parser.add_argument(
        '--verbose', '-v', help='print out log lines', action='store_true')
