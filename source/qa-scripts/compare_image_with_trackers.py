...
import pickle
import argparse
from cv_utils import get_track_frame_id, get_emb, chop_extension, get_distance, \
...
def main(args):
    ...
    input_img = cv2.cvtColor(
        get_image(input_image_id, live_dir), cv2.COLOR_RGB2BGR)
    ...
    for tracker_id in current_trackers:
        ...
        for track_image in this_tracker_imgs:
            if current_frame_id > get_track_frame_id(track_image):
                track_image_id = find_image_id_from_frames_dict(
                    track_image, frames_dict)
                if track_image_id:
                    track_emb = get_emb(track_image_id, live_dir)
                    dist = get_distance(input_emb, track_emb)
                    print(tracker_id, track_image, dist)
                    img_path = os.path.join(trackers_dir, tracker_id,
                                            track_image + '.jpg')
...
def parse_agruments(agrv):
    parser = argparse.ArgumentParser(
        'parses agrument for compare trackers',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '--qa_dir',
        help='QA database dir',
        default='/media/loi/E/Work/EyeQ/Stuff/tracking-data/door_1_qa-database')
    parser.add_argument(
        '--trackers_dir',
        help='tracking results dir',
