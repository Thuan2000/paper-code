...
from cv_utils import get_area, CropperUtils, show_frame
...
def main():
    ...
    if Config.DEBUG:
        ...
        rb.channel.basic_consume(
            on_server_rx_rpc_request, queue=rb.VINGROUP_RPC_SERVER)
...
def detect_and_assign(imgs, ids):
    ...
    for img_index, img in enumerate(imgs):
        ...
        for bbox in bboxes:
            ...
            embedding_dict[img_index].append((emb, display_face_img,
                                              padded_bbox_str, bbox))
        ...
        if img_index == 0:
            ...
            for (emb, display_face_img, padded_bbox_str, bbox) in embs_faces:
                ...
                if dists[min_index] > Config.MIN_ASSIGN_THRESHOLD:
                    ...
                    continue
                id_dict[found_id].append((emb, display_face_img,
                                          padded_bbox_str, get_area(bbox)))
    ...
    sorted_bb_areas = sorted(
        bbox_area_dict.items(), key=operator.itemgetter(1), reverse=True)
    ...
    if len(id_dict.keys()) < nrof_ids:
        ...
        id_dict = dict(
            [(key_dict[k], v) for k, v in id_dict.items() if k in valid_keys])
    else:
...
def prepare_response_message_link(id_dict):
    ...
    for face_id, embs_faces in id_dict.items():
        ...
        for emb, display_face_img, padded_bbox_str, _ in embs_faces:
            ...
            file_relative_path = os.path.join(Config.RPCServer.SAVE_PATH,
                                              file_name)
            file_absolute_path = os.path.join(Config.RPCServer.DST_PATH,
                                              file_relative_path)
