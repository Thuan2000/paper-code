...
def build_simple_block(incoming_layer,
    ...
    net.append((names[0],
                ConvLayer(
                    ...
                    pad,
                    flip_filters=False,
                    nonlinearity=None) if use_bias else ConvLayer(
                        incoming_layer,
                        num_filters,
                        filter_size,
                        stride,
    ...
    if nonlin is not None:
        net.append((names[2], NonlinearityLayer(
            net[-1][1], nonlinearity=nonlin)))
...
def build_residual_block(incoming_layer,
    ...
    net_tmp, last_layer_name = build_simple_block(
        ...
        int(
            lasagne.layers.get_output_shape(incoming_layer)[1] *
            ratio_n_filter), 1, int(1.0 / ratio_size), 0)
    net.update(net_tmp)

    net_tmp, last_layer_name = build_simple_block(
        net[last_layer_name],
        list(map(lambda s: s % (ix, 2, 'b'), simple_block_name_pattern)),
        lasagne.layers.get_output_shape(net[last_layer_name])[1], 3, 1, 1)
    net.update(net_tmp)

    net_tmp, last_layer_name = build_simple_block(
        net[last_layer_name],
        list(map(lambda s: s % (ix, 2, 'c'), simple_block_name_pattern)),
        lasagne.layers.get_output_shape(net[last_layer_name])[1] *
        upscale_factor,
        1,
        1,
        0,
    ...
    if has_left_branch:
        net_tmp, last_layer_name = build_simple_block(
            incoming_layer,
            list(map(lambda s: s % (ix, 1, ''), simple_block_name_pattern)),
            int(
                lasagne.layers.get_output_shape(incoming_layer)[1] * 4 *
                ratio_n_filter),
            1,
            int(1.0 / ratio_size),
            0,
    ...
    net['res%s_relu' % ix] = NonlinearityLayer(
        net['res%s' % ix], nonlinearity=rectify, name='res%s_relu' % ix)
...
def build_model(input_size):
    ...
    for char in block_size:
        if char == 'a':
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1, 1, True, 4, ix='2%s' % char)
        else:
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1.0 / 4, 1, False, 4, ix='2%s' % char)
        net.update(sub_net)
    ...
    for char in block_size:
        if char == 'a':
            ...
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1.0 / 4, 1, False, 4, ix='4%s' % char)
        net.update(sub_net)
    ...
    for char in block_size:
        if char == 'a':
            ...
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1.0 / 4, 1, False, 4, ix='5%s' % char)
        net.update(sub_net)
    net['pool5'] = PoolLayer(
        net[parent_layer_name],
        pool_size=7,
...
class BodyExtractor:
    ...
    def __init__(self):
        ...
        self.im_mean = np.asarray([103.0626238, 115.90288257, 123.15163084],
                                  dtype=np.float32)
        ...
        self.network = lasagne.layers.batch_norm(
        ...
        with np.load(weight_fname) as f:
            ...
            lasagne.layers.set_all_param_values(self.network_out,
                                                self.param_values)
