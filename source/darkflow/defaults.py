class argHandler(dict):
    ...
    def setDefaults(self):
        ...
        self.define('summary', './summary/',
                    'path to TensorBoard summaries directory')
        ...
        self.define(
            'pbLoad', '',
            'path to .pb protobuf file (metaLoad must also be specified)')
        self.define(
            'metaLoad', '',
        ...
        while i < len(args):
            ...
            if isinstance(self.get(argumentName), bool):
                if not (i + 1) >= len(args) and (
                        args[i + 1].lower() != 'false' and args[i + 1].lower()
                        != 'true') and not args[i + 1].startswith('--'):
                    print(
                    ...
                    '--') and not (i + 1) >= len(args) and not args[
                        i + 1].startswith('--') and argumentName in self:
